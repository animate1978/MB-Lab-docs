MB-Dev Framework
================

These are a new set of tools that allow the editing of MB-Lab characters.

The tools allow the editing of existing base mesh as well as creating new base mesh data.

.. image:: images/mb_dev_178.png

The tools are split into two parts.

**Model Edition** which allows the editing of existing base character models

**Model Intergration** which is meant to create the JSON data required for base character models


.. toctree::
   :maxdepth: 2
   
   morph_creator
   copy_morphs
   char_lib_creation
   expression_editor
